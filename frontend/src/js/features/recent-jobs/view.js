...
export function renderRecentJobsList({
  ...
  list.classList.remove("hidden");
  empty.classList.add("hidden");
  list.__pdfManagerRecentJobSelect = onSelect;
  list.__pdfManagerRecentJobDelete = onDelete;
  if (!list.__pdfManagerRecentJobBound) {
    list.__pdfManagerRecentJobBound = true;
    list.addEventListener("click", (event) => {
      ...
      if (confirmButton && list.contains(confirmButton)) {
        ...
        list.__pdfManagerRecentJobDelete?.(item?.dataset.jobId || "");
        return;
      }
      ...
      list.__pdfManagerRecentJobSelect?.(button.dataset.jobId || "");
    });
    list.addEventListener("keydown", (event) => {
      ...
      event.preventDefault();
      list.__pdfManagerRecentJobSelect?.(item.dataset.jobId || "");
    });
  }
