...
const targetPlatform = process.env.PDF_MANAGER_DESKTOP_PLATFORM || process.platform;
const allowBundledMacPython = process.env.PDF_MANAGER_BUNDLE_MAC_PYTHON === "1";
const skipBundledRuntimeVerification = process.env.PDF_MANAGER_SKIP_BUNDLED_RUNTIME_VERIFICATION === "1";
...
const desktopIconSource = path.join(desktopRoot, "assets", "pdf-manager-logo.png");
...
const releaseVersion = (process.env.PDF_MANAGER_VERSION || "").trim()
...
if (!releaseVersion) {
  throw new Error(
    `Missing release version; fallback sources PDF_MANAGER_VERSION, git describe, ${versionFile}, and package.json are all empty`,
  );
}
...
if (!frontendOnly && targetPlatform === "darwin" && hasBundledPosixPython(embeddedPythonRoot)) {
  if (allowBundledMacPython) {
    ...
    console.warn(
      "[prepare-app] skip bundling backend/python for darwin because PDF_MANAGER_BUNDLE_MAC_PYTHON!=1",
    );
  }
}
