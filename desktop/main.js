...
const DESKTOP_API_KEY = "pdf-manager-desktop";
...
function resolveWindowIcon() {
  if (app.isPackaged) {
    return path.join(__dirname, "assets", "PDF Manager-logo.png");
  }
  return path.join(__dirname, "assets", "PDF Manager-logo.png");
}
...
async function startBundledBackend() {
  ...
  if (apiPortBusy) {
    ...
    throw new Error(
      `端口 ${apiPort} 已被其他进程占用，且不是可复用的 PDF Manager 后端。请先关闭占用进程后再启动桌面端。`,
    );
  }
  ...
  const env = {
    ...
    PDF_MANAGER_FONT_PATH: bundledFontPath,
    PDF_MANAGER_TITLE_BOLD_FONT_PATH: bundledTitleBoldFontPath,
    PDF_MANAGER_TYPST_FONT_DIRS: bundledTypstFontDir,
...
function createWindow() {
  ...
  mainWindow.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedURL) => {
    ...
    dialog.showErrorBox("PDF Manager 页面加载失败", detail);
  });
  ...
  mainWindow.webContents.on("render-process-gone", (_event, details) => {
    ...
    dialog.showErrorBox("PDF Manager 渲染进程异常退出", detail);
  });
...
app.whenReady().then(() => {
  ...
  appendDesktopLog("========== PDF Manager desktop startup ==========");
  ...
  createSplashWindow()
    ...
    .catch((error) => {
      ...
      dialog.showErrorBox("PDF Manager startup failed", detail);
      app.quit();
    });
});
