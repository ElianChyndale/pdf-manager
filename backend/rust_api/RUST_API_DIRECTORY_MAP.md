...
- 改后端运行参数、provider 超时/重试、路径和认证配置：
  [`src/config`](/path/to/pdf-manager/backend/rust_api/src/config)
...
1. [`src/app/router.rs`](/path/to/pdf-manager/backend/rust_api/src/app/router.rs)
   先知道有哪些 HTTP 入口。
2. [`src/app/jobs.rs`](/path/to/pdf-manager/backend/rust_api/src/app/jobs.rs)
   再看 jobs 相关依赖是怎么装起来的。
3. [`src/routes/jobs`](/path/to/pdf-manager/backend/rust_api/src/routes/jobs)
   看 route 只是怎么转发。
4. [`src/services/jobs/facade`](/path/to/pdf-manager/backend/rust_api/src/services/jobs/facade)
   看 command/query 用例入口。
5. [`src/services/jobs/creation`](/path/to/pdf-manager/backend/rust_api/src/services/jobs/creation)
   看创建链路的准备、快照、提交、bundle。
6. [`src/job_runner`](/path/to/pdf-manager/backend/rust_api/src/job_runner)
   最后再进 runtime 执行层。
