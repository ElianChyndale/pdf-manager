...
def test_extract_text_items_keeps_primary_text_blocks_and_captions() -> None:
    ...
    assert [item.text for item in items] == ["Body paragraph", "Results", "Table 1. Caption text"]
    assert [item.structure_role for item in items] == ["body", "heading", "table_caption"]
