...
def test_replay_translation_item_returns_result_without_mutating_payload(monkeypatch) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        monkeypatch.setenv("PDF_MANAGER_TRANSLATION_API_KEY", "test-key")
        job_root = Path(tmp)
...
def test_replay_translation_item_recovers_env_api_key_from_job_db(monkeypatch) -> None:
    ...
    monkeypatch.delenv("PDF_MANAGER_TRANSLATION_API_KEY", raising=False)
            ...
            (),
            {
                ...
                "params": type(
                    ...
                    (),
                    {
                        ...
                        "base_url": "https://api.deepseek.com/v1",
                        "glossary_entries": [],
                        "credential_ref": "env:PDF_MANAGER_TRANSLATION_API_KEY",
                    },
                )(),
            },
        )()
...
def test_replay_translation_case_artifact_uses_frozen_page_payload(monkeypatch) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        monkeypatch.setenv("PDF_MANAGER_TRANSLATION_API_KEY", "artifact-key")
