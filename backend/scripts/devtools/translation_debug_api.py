...
DEFAULT_BASE_URL = (
    os.environ.get("PDF_MANAGER_BACKEND_URL", "").strip()
    or os.environ.get("RUST_API_BASE_URL", "").strip()
    or "http://127.0.0.1:41000"
)
DEFAULT_API_KEY = (
    os.environ.get("PDF_MANAGER_BACKEND_KEY", "").strip()
    or os.environ.get("RUST_API_KEY", "").strip()
    or "pdf-manager-desktop"
)
    ...
    *,
    ...
    request = Request(
        ...
        headers={
            ...
            "X-API-Key": api_key,
            "User-Agent": "pdf-manager-translation-debug-api",
        },
    )
    try:
