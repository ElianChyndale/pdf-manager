...
class BBoxTextStripCandidateAccumulator:
    ...
    page_target_rects: dict[int, tuple[tuple[float, float, float, float], ...]] = field(default_factory=dict)
    ...
    def add_page_plan(self, page_idx: int, page_plan: BBoxTextStripPagePlan) -> None:
        if page_plan.target_rects:
            self.page_target_rects[page_idx] = tuple(rect_tuple(rect) for rect in page_plan.target_rects)
    ...
    def build(self) -> BBoxTextStripCandidates:
        return BBoxTextStripCandidates(
            ...
            page_target_rects=self.page_target_rects,
