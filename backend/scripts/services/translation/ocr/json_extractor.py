...
def _is_primary_translatable_text_block(block: dict, data: dict) -> bool:
    ...
    derived_structure_role = DERIVED_STRUCTURE_ROLE_MAP.get(derived_role, "")
    effective_structure_role = structure_role or derived_structure_role
    if effective_structure_role in PRIMARY_TRANSLATABLE_STRUCTURE_ROLES:
        return True
    if layout_role in {"title", "heading", "caption", "footnote"}:
        return True
    if semantic_role == "abstract":
        return True
    if semantic_role not in PRIMARY_TRANSLATABLE_SEMANTIC_ROLES:
        return False
    if effective_structure_role not in PRIMARY_TRANSLATABLE_STRUCTURE_HINTS:
        return False
    return layout_role in {"paragraph", "list_item", "unknown", ""}
