...
PDF Manager 支持 PaddleOCR、MinerU 和后续可能新增的 OCR provider。不同 provider 的原始 JSON 字段、文件结构、语义标签都不一样。如果翻译和渲染直接读取 provider raw payload，后续每接一个 provider 都会把私有字段扩散到全链路。
